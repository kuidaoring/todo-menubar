...
function Task() {
  ...
  const startDebounceMemoUpdate = (value: string) => {
    ...
    setMemoDebounceTimer(
      setTimeout(() => {
        updateMemoMutation.mutate({ id: query.data?.id, memo: value })
      }, 500)
    )
  }
  useEffect(() => {
  ...
  return (
    <div className={`w-80 flex flex-col h-[calc(100dvh-1.5rem)]`}>
      <div className="flex flex-row m-1 px-2 text-xs text-gray-500">
        ...
        <button
          onClick={() => {
            confirm('タスクを削除しますか？') && deleteTaskMutation.mutate(query.data?.id)
          }}
        >
          🗑
        </button>
      ...
      <div className="overflow-scroll">
        <div className="flex items-center group mr-1 p-1.5 rounded-lg hover:bg-gray-100">
          <input
            ...
            onChange={() => {
              toggleCompletedMutation.mutate(query.data?.id)
            }}
          />
          <h3
            ...
            return (
              ...
              >
                <input
                  ...
                  onChange={() =>
                    toggleStepCompletedMutation.mutate({ id: query.data?.id, stepId: step.id })
                  }
                />
                <p
            ...
            <li className="flex gap-x-3.5 py-2 px-2.5 rounded-lg hover:bg-gray-100">
              <label className="cursor-pointer group w-full">
                ...
                <input
                  ...
                  onChange={() => {
                    toggleIsTodayMutation.mutate(query.data?.id)
                  }}
                />
              </label>
            </li>
